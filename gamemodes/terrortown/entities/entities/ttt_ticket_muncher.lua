...
function ENT:Initialize()
    ...
    self:SetHealth(200)
    self:SetColor(Color(255, 178, 0, 255))
...
if SERVER then
    ...
    function ENT:Touch(entity)
        if entity.Base == "base_ammo_ttt" then
            ...
            local consumed = self:Eat(given)
            entity.AmmoAmount = entity.AmmoAmount - consumed
            if
                entity.AmmoAmount > 0
                and math.ceil(entity.AmmoEntMax * 0.25) <= entity.AmmoAmount
            then
                return
            end
            entity:Remove()
    ...
    function ENT:Digest(digestRate)
        ...
        if self:GetDigestedAmmo() >= self.ProgressToCredits then
            credited = math.min(
                self.DigestToCreditRate,
                (self:GetDigestedAmmo() - (self:GetDigestedAmmo() % self.ProgressToCredits))
                    / self.ProgressToCredits
            )
            -- print("credited:", credited, self:GetDigestedAmmo() % self.ProgressToCredits, self:GetStoredCredits() )
            self:SetDigestedAmmo(
                math.max(0, self:GetDigestedAmmo() - (credited * self.ProgressToCredits))
            )
...
-- @hook
-- @realm server
function GAMEMODE:TTTPlayerUsedTicketMuncher(ply, ent, credited) end
...
function ENT:Use(ply)
    local t = CurTime()
    if not IsValid(ply) or not ply:IsPlayer() or not ply:IsActive() or t < self.NextCredit then
        return
    end
...
if SERVER then
    ...
    function ENT:WasDestroyed()
        ...
        LANG.Msg(originator, "ttt_ticket_muncher_broken", nil, MSG_MSTACK_WARN)
    end
else
    ...
    hook.Add("TTTRenderEntityInfo", "HUDDrawTargetIDTicketMuncher", function(tData)
        ...
        if
            ...
            or ent:GetClass() ~= "ttt_ticket_muncher"
        then
            return
        end
        ...
        tData:SetSubtitle(ParT("ttt_ticket_muncher_subtitle", key_params))
        ...
        if ttt_ticket_muncher_fullness > 0 or ttt_ticket_muncher_digested > 0 then
            ...
            tData:AddDescriptionLine(TryT("ttt_ticket_muncher_empty"), COLOR_ORANGE)
        end
